...
**请不要使用 FOMO 大号登录。** 轮询和推送过快可能触发风控、导致账号被封禁，建议专门注册一个小号来监控。

推荐的轮询配置（在 `.env` 里修改，改完重启生效）：

```env
# ---------- 轮询 ----------
# 轮询间隔(秒)。不要设得太小：请求太频繁容易被限流甚至封号；
# 设得比单轮耗时还短，日志还会一直刷 skipped ... max instances（数据不丢，但看着像故障）
FOMO_POLL_INTERVAL_SEC=27
# /add 新用户时回填多少条历史交易，用来判断"首次买入"
FOMO_BACKFILL_MAX_ITEMS=500
# 推送间隔(秒/条)。一次 5 条以内立刻发完，更多时按这个速度匀速发
FOMO_SEND_INTERVAL_SEC=1.5
# 拉取并发线程数。不建议调大，并发太高会被服务端限流，反而更慢
FOMO_FETCH_WORKERS=12
```
...
|---|---|
| 🌱 / 🟢 / 🔴 | 首次建仓 / 加仓 / 卖出 |
| #标签 | 你用 `/tag` 给这个人打的标签，点一下能筛出同标签的所有推送 |
| 📝 | 币的英文全名与中文名（自动翻译） |
| 💰 📦 📊 📈 | 本笔金额、当前持仓、均价、盈亏 |
| 💎 | 当前市值 |
...
| `/tag <handle> <标签…>` | 给这个人打标签（如 `底部选手 盈利10w`），推送标题里显示成 #话题；`/tag` 看全部；`/untag <handle> [标签]` 去掉 |
| `/hot` | 名单成员最近都在买什么（可加 `今日` / `3日` / `7日`） |
| `/who <合约地址>` | 名单里谁买过这个币 |
| `/ca <合约地址>` | 查这个币：FOMO 用户的观点、持仓与盈亏 |
| `/chips <合约地址>` | 筹码分布：FOMO 平台与 pump.fun 上各持有多少，你的名单里有谁；FOMO 前 10 名持有人的占比、粉丝、投资组合、7 天盈亏 |
...
| `FOMO_POLL_INTERVAL_SEC` | `15` | 多少秒检查一次，**推荐 `27`**（见开头「使用前必读」），名单人多时可再调大 |
