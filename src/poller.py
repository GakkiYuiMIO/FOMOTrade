...
class Poller:
    ...
    def _maybe_sample_price_history(self, conn) -> None:
        ...
        ⚠️ 判断刻意写成 `== n // 2` 而不是常见的 `== 0`:
           采样默认每 20 轮一次(15s×20=5 分钟),与 _maybe_poll_pnl 的
           _PNL_EVERY_N_TICKS(=20)**同一个周期**,而且两者在 tick() 里都排在
           _fetch_snapshots 把 _tick_no 自增**之后**检查 —— 不像 _poll_feed 与
           _maybe_poll_pnl 那样天然靠调用时机差一拍错开,这里比的是同一个
           _tick_no 值。若也写成 `% n == 0`,会在**每一次**该采样的轮次上都跟
           盈亏采集同时触发,不是偶尔撞上,是必然同时——两个低频任务叠在同一
           tick 上,和 _FEED_EVERY_N_TICKS/_PNL_EVERY_N_TICKS 的错峰设计初衷背道而驰。
           错开半个周期后,采样固定落在盈亏采集之后第 n//2 轮,只要 n>1 就
           永远不会撞见(n=1 时退化成每轮都采,那种配置下"撞不撞"已经没有意义)。
           见 tests/test_poller.py::test_价格采样与名单盈亏永不同轮触发。
        ⚠️ 采样本身不打网络请求,与盈亏采集撞车的真实代价只是"同一 tick 多做
           一次 executemany",量级上远小于网络请求的排队等待,这里仍然选择错开
           纯粹是因为免费(改一个比较符号),不是因为不错开会有明显的性能问题。
        """
