...
# 那么这条信号的时效要求就和买入推送一样 —— 轮转一圈(⌈人数/每轮批量⌉ ≈ 20 轮,
# 按生产 27s 间隔约 9 分钟)的覆盖延迟太久了。
#
# ⚠️ 成本必须按**峰值**算,不能按均值(这是上一版转账采集踩过的坑)。
#   本地模拟(98 人名单 = 当前生产规模 · fomo_fetch_workers=12 · 60 轮 ·
#   数的是整轮全部请求,取后 40 轮避开冷启动的 balances 预热):
#     K=0   单轮峰值 111 个请求 → ⌈111/12⌉ = 10 波
#     K=12  单轮峰值 123 个请求 → ⌈123/12⌉ = 11 波   (其中转账 5 → 17)
#   即**每轮最多多 K 个请求**,一个不多(与轮转批次重合的部分是白赚的,
#   但保护按最坏情况定,不按期望值)。
#   按本项目实测的 p50 0.3~1.2s/请求折算:单轮 3.0~12.0s → 3.3~13.2s,峰值 +1.2s。
#   上限取 12 就是这么来的:**恰好多一整波**(12 = 默认并发度),
#   轮询间隔(默认 15s、生产当前 27s)仍有余量。
