...
复用 name_glossary(kind/key/value/expires_at 的通用 KV,见 store):
  ...
  holders       **内存** 90 秒(实测上游自己的缓存就是这个量级)。刻意不落库:
                ...
                ⚠️⚠️ **代价(H6,如实记下来)**:转入推送(/tin、转入聚合)走的是
                `cached()` 这条**只读缓存、不发请求**的路,而 90 秒的内存缓存基本
                不可能正好命中一条转入 —— 于是**转入推送实际上几乎永远没有
                🧑‍🤝‍🧑 那一行**(🚀 有,因为发射台是永久落库的)。
                考虑过给 holders 也落 SQLite,**否掉了**:
                  · 短 TTL(90 秒)落库 → 转入那条路照样命中不了,只是多了磁盘写;
                  · 长 TTL(小时级)落库 → 转入推送里印的是几小时前的持有人数。
                    持有人数是**快变量**,而这条消息紧挨着一笔刚发生的转账 ——
                    印一个陈旧的数字就是印一句假话(§10.3:错的比没有更糟)。
                ⇒ 保持现状:转入推送不带这一行,并在 README 里如实写明,
                  不做"看起来有、其实是旧的"那种半吊子。
