...
def tick() -> None:
    ...
    ⚠️ 唯一的结构性约束：不允许边落库边推送。
       ...
       代价：推送延迟从「拉到即发」变成「tick 末尾统一发」，最坏 +一个 tick。明确接受。
       （提速后单轮 3-5s、间隔 12s，这个代价已经从 +20s 缩到 +5s 以内。）
    """
    # --- 1) 每 tick 最多为一个新用户建立历史基线 ---
    seed_next_pending_user()

    # --- 2) 采集 ---
    # ⚠️ 2026-08-11 改成增量采集，不再全员全端点拉。详见 tasks/todo.md「单轮提速」。
    #    一次 /feed/tradingActivity（关注流，0.35s）问出"谁刚动过" → 只给这些人拉 swaps；
    #    其余人靠每轮 12 个的兜底轮转扫描覆盖（关注流只覆盖"已关注"的人，这条兜底
    #    是正确性要求不是优化）；balances 走进程内缓存 + 有交易当场刷 + 每轮 8 个轮转。
    #    结果：每轮 API 调用 ~232 次 → ~46 次，单轮 32-45s → 3-5s。
    raw = {u["user_id"]: fetch_incremental(u) for u in list_active_users()}
