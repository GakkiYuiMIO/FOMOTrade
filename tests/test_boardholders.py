...
class Test榜单单飞闸:
    ...
    def test_拿不到闸不算失败所以不写负缓存(self):
        ...
        # ⚠️⚠️ 关键的一句:**在前一个 job 写回之前**,缓存必须一个字节都没被碰过。
        #    只看"后面还能不能拿到榜"是**抓不到**这条的 —— 前一个 job 随后那次成功写回
        #    会把撞闸写下的负缓存原样盖掉(变异跑实测:那样写的话全量一条都不红)。
        #    真正的危险在时序上:撞闸这条路径压根不该有"写"这个动作。
        assert cache._until == 0.0, "撞闸这条路径绝不许碰缓存状态(负缓存也不行)"
        release.set()
        t.join(5.0)
