...
class Test底池对手:
    def test_pump这两条链上这一行一律不出现(self, db, cfg):
        """
        ⚠️⚠️ **行为变更,如实记在这里**:这一行只在对手是**币股**时才出现,
           而币股判据只在 Robinhood 链上找到了可靠标记(name 以 " • Robinhood Token"
           结尾)。pump.fun 的币在 solana / bsc / base 上,那几条链上
           `SPCXB · SpaceX` 这类代币化股票**确实存在**,但 247 个真实样本里
           找不到任何一个能把它和 memecoin 分开的标记 ——
           `BTCB` 也以 B 结尾、`SpaceX`/`GameStop` 是谁都能取的名字。
           按"宁可漏掉一个股票,也不能把不是股票的说成股票"的口径,这里整行不出现。
           判据补上的那天,这一行会自动回来(见 dexscreener.STOCK_NAME_MARKERS)。
        """
        seeded_user()
        client = FakeClient(
            portfolios={HEX_SVM: position_payload(
    ...
    def test_第三方字符串一路到推送都不会漏出去(self, db, cfg):
        """
        ⚠️ 转义/截断的防线钉在渲染层(test_formatter 里那几条)。这里只钉一件事:
           一个带 `<script>` 的上游名字**不会**从 pump 这条路径漏进推送。
        """
        seeded_user()
        client = FakeClient(
            portfolios={HEX_SVM: position_payload(
